...
async function handleFile(file) {
  ...
  try {
    ...
    downloadInfo.innerHTML =
      `<b>${total}</b> <code>.h</code> files \u2014 ` +
      `each is a <code>uint16_t</code> RGB565 array of ${outW}\u00d7${outH} = ${(outW*outH).toLocaleString()} pixels.`;
...
function buildIndexHeader(totalFrames, width, height) {
  const ls = [
    `// frames_index.h \u2014 auto-generated by GIF\u2192C Array Splitter`,
    `// ${totalFrames} frames, each ${width}\u00d7${height} RGB565 pixels`,
    ``,
    `#pragma once`,
    ``,
  ...
  for (let i = 0; i < totalFrames; i++) ls.push(`#include "frame_${pad(i)}.h"`);
  ls.push(``, `// Pointer array for easy iteration`);
  ls.push(`const uint16_t* const frames[FRAME_COUNT] PROGMEM = {`);
  for (let i = 0; i < totalFrames; i++) ls.push(`  frame_${pad(i)},`);
  ls.push(`};`);
  return ls.join("\n");
}
...
dlZipBtn.addEventListener("click", async () => {
  ...
  const zip  = new JSZip();
  for (const { name, content } of frames) zip.file(name, content);

  const outW = parseInt(outWidthEl.value)  || 240;
  const outH = parseInt(outHeightEl.value) || 240;
  zip.file("frames_index.h", buildIndexHeader(frames.length, outW, outH));
  ...
  dlZipBtn.textContent = "\u2b07 Download all frames as ZIP";
});
...
dlIndexBtn.addEventListener("click", () => {
  ...
  const outH = parseInt(outHeightEl.value) || 240;
  triggerDownload(
    new Blob([buildIndexHeader(frames.length, outW, outH)], { type: "text/plain" }),
    "frames_index.h"
  );
});
